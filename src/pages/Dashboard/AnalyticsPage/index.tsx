...
const AnalyticsPage: React.FC = () => {
  ...
  const calculateMetrics = () => {
    ...
    interactions.forEach(i => {
      ...
      if (s in baseMetrics.sentiments) baseMetrics.sentiments[s as keyof typeof baseMetrics.sentiments]++;
    });
    ...
    const tokensUsed = assistant.assistant_token_usage || 0;
    const tokensAvailable = assistant.assistant_token_limit || 1;
    const tokenUsagePercentage = Math.round((tokensUsed / tokensAvailable) * 100);

    // Calcula média de tokens por interação (se houver interações)
    const avgTokens = interactions.length > 0 
      ? Math.round(interactions.reduce((sum, i) => sum + (i.total_tokens || 0), 0) / interactions.length)
      : 0;
          ...
          <Card sx={{ mb: 1 }}>
            <CardContent>
              <Stack direction="row" justifyContent="space-between" alignItems="center">
                ...
                <Box textAlign="right">
                  <Typography variant="body2">
                    Tokens usados: {metrics.tokensUsed.toLocaleString()} de {metrics.tokensAvailable.toLocaleString()}
                  </Typography>
                  <Typography variant="body2">
