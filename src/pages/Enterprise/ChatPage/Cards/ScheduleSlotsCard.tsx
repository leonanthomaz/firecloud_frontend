          ...
          {Object.entries(slotsByDay).map(([date, daySlots]) => (
            <Box key={date}>
              ...
              <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1 }}>
                {daySlots
                  .filter(slot => slot.id || slot.public_id)
                  .map((slot) => (
                    <Button
                      key={slot.id || slot.public_id}
                      variant="outlined"
                      size="small"
                    ...
                    >
                      {(slot.start ? formatTime(slot.start) : 'Indisponível')} - {(slot.end ? formatTime(slot.end) : 'Indisponível')}
                    </Button>
                  ))}
              </Stack>
